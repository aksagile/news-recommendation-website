...
from apikeyspath import TW_TOKEN_KEY, TW_TOKEN_SECRET_KEY, TW_CON_SECRET_KEY, TW_CON_KEY
...
class Predictor(object):
    ...
    def __init__(self, model_pickle, tfidf_pickle):
        ...
        self.model = pickle.load(open(PATH_TO_REPO + "data/" + model_pickle,"rb"))
        self.tfidf = pickle.load(open(PATH_TO_REPO + "data/" + tfidf_pickle,"rb"))

        # Label dictionary for nice categories
        self.label_dict = {0: "Arts", 1: "Business",  2: "Health", 3: "Science", 4: "Sports", 5: "Tech", 6: "Travel", 7: "World"}
             #2: "Food", 4: "NY", 5: "Politics", 6: "RealEstate",, 9: "Style" 12: "US",
        # Label dictionary for categories as they need to be put into the NYT Top Stories API
        self.label_dict_NYT = {0: "arts", 1: "business", 2: "health", 3: "science",  4: "sports", 5: "tech", 6: "travel", 7: "world"}
            
        # "dining", 3:,4: "nyregion", 5: "politics", 6: "realestate", 7:8: "sports", 9: "fashion", 10: "technology", 11: "travel", 12: "national", 13: "world"}
        # Set up the Twitter access
        auth = tweepy.OAuthHandler(TW_CON_KEY,TW_CON_SECRET_KEY)
        auth.set_access_token(TW_TOKEN_KEY, TW_TOKEN_SECRET_KEY)
    ...
    def recommend_article(self, tweets, labels):
        ...
        for label in labels:
            ...
            #print(articles)
            #print(".............its the respnse.........",str(articles))
            ...
            print("arg sorted..........",argsortedarray)
            ...
            sentencestarts = ["You are probably", "It seems like you are also", "However, you are possibly also", "Furthermore, you could even be"]#,"Also you might be"
            if counter > 4:
            ...
            for i in range(0,5):
                print("Maybe you find the following article from this topic interesting...\n")
                tweetList.append({"tweet":{"title":HTMLParser().unescape(articles["results"][argsortedarray[i]]["title"]),"abstract":HTMLParser().unescape(articles["results"][argsortedarray[i]]["abstract"]),"url":HTMLParser().unescape(articles["results"][argsortedarray[i]]["url"])}})
                #print("TITLE:\n" + HTMLParser().unescape(articles["results"][argsortedarray[i]]["title"]) + "\n")
                #print("ABSTRACT:\n" + HTMLParser().unescape(articles["results"][argsortedarray[i]]["abstract"]) + "\n")
                #print("URL:\n" + HTMLParser().unescape(articles["results"][argsortedarray[i]]["url"]) + "\n\n")
            print(tweetList)
